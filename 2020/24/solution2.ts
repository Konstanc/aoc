...
function splitPaths(spaths: string[][]) {
    let paths: string[][] = [];
...
function getBlackTiles(paths: string[][]) {
    let blacks = {};
    paths
...
function calcBlacks(field: boolean[][]) {
    let res = 0;
...
function play(field: boolean[][], times) {
    for (let i = 0; i < times; i++) {
        field = getNextField(field);
    }
    return field;
}
function calcNeig(field: boolean[][], x: number, y: number) {
...
function getNextField(field: boolean[][]) {
