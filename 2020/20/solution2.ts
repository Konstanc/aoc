...
type Tile = { id: string; subId?: string; image: string[][]; edges?: string[][]; subTiles?: Tile[] };
let size = 3;
...
function sameEdge(toDown, t1: Tile, t2: Tile) {
...
function fit(x: number, y: number, image: Tile[][], tiles: string[]): Tile[][] | false {
    let possibleTiles: { id: string, subId: string }[] = [];
    if (x == 0 && y == 0) {
        ...
        if (x == 0) {
            ...
            if (y != 0) {
                ...
                possibleTiles = possibleTiles.filter(pTile =>
                    pTile.subId in tileUpIndex);
            }
        }
    }
...
function fitSquares(tiles: Tile[]) {
    let x = 0;
    let y = 0;
    let image: Tile[][] = Array.from(Array(size), a => []);
...
let globalToDownIndex: TIndex = {};
let globalToRightIndex: TIndex = {};
let allTileIndex: { [key: string]: Tile; } = {};
let allSubTileIndex: { [key: string]: Tile; } = {};
function fillGlobalIndex(tiles: Tile[]) {
    tiles.forEach(tile => {
...
function getFullImage(image: Tile[][]) {
...
function getAllFullOrs(image: string[][]) {
    let res: string[][][] = [];
    let l = image.length;
    for (let i = 0; i < 4; i++) {
        for (let j = 0; j < 2; j++) {
...
function isMonster(cx: number, cy: number, image: string[][], monster: string[][], safe: string[][]) {
...
function findMonsters(image: string[][]) {
    let monster = [
        '                  # ',
        '#    ##    ##    ###',
        ' #  #  #  #  #  #   '
...
function countSafe(image: string[][]) {
...
function plotImage(image: string[][]) {
