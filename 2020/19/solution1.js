...
function matchRuleList(s, ruleList, baseRules, rules) {
...
let matchIndex = {};
function match(s, ruleIndex, baseRules, rules) {
    if (!(s in matchIndex)) {
        matchIndex[s] = [];
    }
...
function match_(s, ruleIndex, baseRules, rules) {
    if (baseRules[ruleIndex] == s)
        return true;
    if (baseRules[ruleIndex])
        return false;
