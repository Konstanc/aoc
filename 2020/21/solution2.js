...
function possibleForSearch(alg, ing, solution, foods) {
...
function purgeFreeIngs(freeIngs, sFoods) {
...
function possibleSearch(alg, ing, foods) {
    let newFoods = foods.map(({ ings, algs }) => ({ ings: ings.slice(), algs: algs.slice() }));
    for (let i = 0; i < newFoods.length; i++) {
        let food = newFoods[i];
        let algIndex = food.algs.indexOf(alg);
...
function getSolution(curSolution, foods) {
    if (foods.length == 0) {
        return curSolution;
    }
    let food = foods[0];
...
function solve(input) {
    ...
    let solution = getSolution([], dFood.sort((a, b) => a.algs.length - b.algs.length));
