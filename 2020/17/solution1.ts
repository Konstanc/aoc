...
function emptyField(lx: number, ly: number, lz: number) {
    let field: boolean[][][] = [];
...
function calcNeighbors(field: boolean[][][], cx: number, cy: number, cz: number) {
    let lz = field.length;
...
function isActive(field: boolean[][][], x: number, y: number, z: number) {
    let lz = field.length;
    let ly = field[0].length;
    let lx = field[0][0].length;
