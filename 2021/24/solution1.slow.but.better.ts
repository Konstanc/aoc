...
function calcRes(data: ReturnType<typeof prepare>) {
    ...
    let res = execFrom(0, [0, 0, 0, 0], data).join('');

    return res;
}

let iExec = {};
function execFrom(i, prevRegs: number[], data: ReturnType<typeof prepare>) {
    if (i >= data.length) {
        if (prevRegs[3] == 0) {
            return [];
        } else {
    ...
    switch (d.op) {
        case 'inp':
            for (let j = 9; j >= 1; j--) {
                regs = [...prevRegs];
                regs[resInd] = j;
                let pRes = execFrom(i + 1, regs, data);
                if (pRes !== false) {
                    return [j, ...pRes];
                }
            }
    ...
    let res = execFrom(i + 1, regs, data);
    if (storeRes) {
        iExec[ind] = res;
    }
    return res;
